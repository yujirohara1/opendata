...
def tryAunthenticationRedmine(apikey, usernm):

  try:
    envVariable = EnvVariable.query.filter(
      EnvVariable.key=='REDMINE_URL_KEY',
      EnvVariable.code=='0'
    ).all()

    for env in envVariable:
      try:
        redmine = Redmine("https://my.redmine.jp/ryu-etsu/", key=apikey) #APIキーで認証を試みる session["redmine_url"]
      
        if redmine.auth().login == usernm : #ユーザIDも一致する
          id = random.randint(1, 9999999999999999)
          users[id] = User(id, id, request.form["user_nm"],request.form["api_key"]) 
          login_user(users[id])
          session["api_key"] = apikey
          session["user_nm"] = request.form["user_nm"]
...
def login():
  ...
  if(request.method == "POST"):
    ...
    try:
      ...
      if tryAunthenticationRedmine(request.form["api_key"], request.form["user_nm"]):
